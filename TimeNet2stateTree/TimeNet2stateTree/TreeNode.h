...
typedef struct Node 
{
	...
	struct Node *PtrBackOldNext;//链接下个反向旧节点下个地址
	struct Node *PtrBackOldBefore;//链接上个反向旧节点下个地址

	struct Node *PtrBackNotDoneNext;//链接下个未扩展节点下个地址
	struct Node *PtrBackNotDoneBefore;//链接上个未扩展节点下个地址
	bool MultiSon;//重复计算判断
...
typedef struct Tree
{
	...
	Node *PtrBackOldHead;//旧结点头结点
	Node *PtrBackOldTail;//旧结点尾结点
