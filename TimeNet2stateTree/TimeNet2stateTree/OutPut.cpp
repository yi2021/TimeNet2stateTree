...
void output(Tree *Tr)
{
	...
	NetState *imf;
	P = (*Tr).PtrHead;
	fp = fopen("E:\\reinforce learning\\TimeNet2stateTree\\output\\back\\111.txt", "w+");
	printf("%s", "输出结果ID");
	printf("\n");
...
void outputpy(Tree* Tr)
{
	...
	struct Node* P;
	P = (*Tr).PtrHead;
	fp = fopen("E:\\reinforce learning\\TimeNet2stateTree\\output\\py\\111.txt", "w+");
	printf("%s", "输出结果PY");
	printf("\n");
	do
	{
...
void outback(Tree *Tr)
{
	...
	struct Node *P;
	P = (*Tr).PtrHead;
	fp = fopen("E:\\reinforce learning\\TimeNet2stateTree\\output\\out\\111.txt", "w+");
	if (fp == NULL)
	{
		cout << "文件打开失败" << endl;
		exit(1);
...
//	struct Node *P;
//	P = (*Tr).PtrHead;
//	fp = fopen("F:\\github\\T2S\\TimeNet2stateTree\\Data\\outPut\\backh\\222mxth-change.txt", "w+");
//	do
//	{
//		for (i = 0; i < (*Tr).place_num; i++)
//		{
...
void OutputBackHTrans(Tree *Tr)
{
	...
	struct Node *P;
	P = (*Tr).PtrHead;
	fp = fopen("E:\\reinforce learning\\TimeNet2stateTree\\output\\backh\\111.txt", "w+");
	do
	{
		for (i = 0; i < (*Tr).place_num; i++)
		{
